...
public class FurnitureAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(position==0){
           FurnitureViewPagerViewHolder h=(FurnitureViewPagerViewHolder)holder;
            h.setShoppingViewPagerData(itemsOfViewpager.get(position));
            return ;
        }
        position--;
       FurnitureViewHolder h=(FurnitureViewHolder)holder;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType==VIEW_TYPE_VIEWPAGE){
            ...
            return new FurnitureViewPagerViewHolder(view,LayoutInflater.from(parent.getContext()));
