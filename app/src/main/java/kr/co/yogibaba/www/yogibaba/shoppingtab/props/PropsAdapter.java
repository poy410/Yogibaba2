...
public class PropsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (position == 0) {
            PropsViewPagerViewHolder h = (PropsViewPagerViewHolder) holder;
            h.setShoppingViewPagerData(itemsOfViewpager.get(position));
            return;
        }
        position--;
        PropsViewHolder h = (PropsViewHolder) holder;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_VIEWPAGE) {
            ...
            return new PropsViewPagerViewHolder(view, LayoutInflater.from(parent.getContext()));
    ...
    PropsViewHolder.OnItemClickListener mListener;

    public void setOnItemClicListener(PropsViewHolder.OnItemClickListener listener) {
        mListener = listener;
    }
}
